...
class MedicineBatch(BaseModel):
    ...
    quantity = models.PositiveIntegerField(
        ...
        db_column="quantity",
        help_text="Số lượng nhập theo đơn vị cơ sở",
    )
    remaining_quantity = models.PositiveIntegerField(
        null=False,
        default=0,
        db_column="remaining_quantity",
        help_text="Số lượng còn lại theo đơn vị cơ sở",
    )
    import_price_per_base_unit = models.DecimalField(
        max_digits=12,
        decimal_places=2,
        null=True,
        blank=True,
...
class SearchKeyword(BaseModel):
    """Từ khóa tìm kiếm — theo dõi lượt tìm để hiển thị 'Tìm kiếm phổ biến'."""

    keyword = models.CharField(max_length=120, null=False, blank=False, db_index=True, db_column="keyword")
    keyword_lookup = models.CharField(
        max_length=120,
        null=False,
        blank=False,
        unique=True,
...
class Category(BaseModel):
    ...
    slug = models.CharField(
        ...
        blank=True,
        db_index=True,
        help_text="URL slug cho category (unique per parent) - auto-generated",
    )
    ...
    path_slug = models.CharField(
        ...
        unique=True,
        db_index=True,
        help_text="category.categorySlug (unique)",
    )
...
class ProductVariant(BaseModel):
    ...
    base_unit = models.CharField(
        ...
        blank=True,
        db_index=True,
        help_text="Đơn vị cơ sở nhỏ nhất, ví dụ: viên, gói",
    )

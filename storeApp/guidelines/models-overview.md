...
- **Scrape data dirs**: `storeApp/test/data/new/` (có `saleUnits`), `storeApp/test/data/old/` (chủ yếu `packageOptions`). **Giá thiếu/0**: `store_import_pricing.ensure_unit_pricing` — infer từ unit cùng variant → fallback row → smart random theo tier (viên/gói/hộp…) × `quantity_in_base` (mặc định bật; `--no-smart-random-price` = random phẳng). **CLI**: `store_import_refactor` / `store_import_csv`; batch qty ≈ `quantity_in_base × random(--batch-pack-mult-*)`.
- **Kiểm tra catalog sau import**: `python manage.py store_audit_product --overview` (thống kê DB) và `--mid <sku> [--scrape-root …]` (so CSV). Schema JSON in `--overview` trùng mục **§9** dưới đây.

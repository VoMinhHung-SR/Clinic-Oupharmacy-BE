...
#         addr = examination.user.addresses.filter(is_default=True).first() or examination.user.addresses.first()
#         distance_text = duration_value = None
#         if addr and addr.lat is not None and addr.lng is not None:
#             res = requests.get('https://rsapi.goong.io/Direction', ...)
#             # parse distance/duration from res_data
#     ...
